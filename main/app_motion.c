...
esp_err_t app_motion_format_card(void)
{
    ...
    /* Nothing may hold a file open across the format. */
    if (s_mo.clip) {
        close_clip();
    }
    s_mo.stats.armed = false;
    vTaskDelay(pdMS_TO_TICKS(200));

    ESP_LOGW(TAG, "formatting %s — all clips and sounds are being erased",
             CONFIG_BSP_SD_MOUNT_POINT);
    err = esp_vfs_fat_sdcard_format(CONFIG_BSP_SD_MOUNT_POINT, bsp_sdcard_get_handle());
    if (err != ESP_OK) {
    ...
    if (mkdir(CLIPS_DIR, 0777) != 0 && errno != EEXIST) {
        ESP_LOGE(TAG, "cannot recreate %s: %s", CLIPS_DIR, strerror(errno));
    }
    if (mkdir(CONFIG_BSP_SD_MOUNT_POINT "/sounds", 0777) != 0 && errno != EEXIST) {
        ESP_LOGE(TAG, "cannot recreate the sounds directory: %s", strerror(errno));
    }

    if (esp_vfs_fat_info(CONFIG_BSP_SD_MOUNT_POINT, &total, &freeb) == ESP_OK) {
        s_mo.stats.free_bytes = freeb;
        ESP_LOGI(TAG, "formatted: %lluMB free of %lluMB",
                 (unsigned long long)(freeb / (1024 * 1024)),
                 (unsigned long long)(total / (1024 * 1024)));
    }

    s_mo.stats.events = 0;
    s_mo.stats.clips_written = 0;
    s_mo.stats.clips_deleted = 0;
    s_mo.stats.armed = true;
    return ESP_OK;
}
